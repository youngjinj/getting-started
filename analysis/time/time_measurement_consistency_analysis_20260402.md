...
- 목적: 경과 시간(elapsed time) 측정 및 스레드 대기(pthread_cond_timedwait)에 monotonic clock을 일관되게 사용하도록 전수 조사
...
CUBRID 엔진에서 경과 시간(elapsed time) 측정과 스레드 대기(pthread_cond_timedwait)에
non-monotonic clock을 사용하고 있다. non-monotonic clock은 NTP 동기화, 수동 시계 변경,
VM 마이그레이션 등 외부 요인에 의해 시간이 앞/뒤로 점프할 수 있어
경과 시간이 음수이거나 비정상적으로 큰 값이 되고, 스레드 대기 시간이 왜곡될 수 있다.

영향 범위는 **성능 통계 수집, 스레드 sleep/wake 제어, 로그 복구 성능 추적,
lockfree 해시맵 연산 측정, lock 대기 timeout, deadlock 검출 간격,
pthread_cond_timedwait 스레드 대기, heartbeat 노드 관리** 등 엔진 핵심 경로 전반에 걸쳐 있다.

### 핵심 변경

경과 시간 측정 및 스레드 대기에 사용되는 clock을 monotonic clock으로 통일한다.

| 변경 | 현재 (non-monotonic) | 변경 후 (monotonic) |
|------|---------------------|-------------------|
| C API (tsc fallback) | `CLOCK_REALTIME_COARSE` | `CLOCK_MONOTONIC` |
...
| C++ 기타 경과 시간 | `std::chrono::system_clock` | `std::chrono::steady_clock` |
| pthread_cond_timedwait | `CLOCK_REALTIME` (기본값) | `CLOCK_MONOTONIC` (`pthread_condattr_setclock`) |
| C 경과 시간 (엔진) | `gettimeofday` / `time(NULL)` | `clock_gettime(CLOCK_MONOTONIC)` |
| C 경과 시간 (heartbeat) | `gettimeofday` | `clock_gettime(CLOCK_MONOTONIC)` |
...
| 3 | pthread_cond_timedwait CLOCK_MONOTONIC 전환 + timeout 변경 | 7 | 10 | 낮음 |
| 4 | C++ system_clock 추가 분 (log_recovery, query_opfunc, log_manager, server_support) | 4 | 4 | 낮음 |
| 5 | C gettimeofday 엔진 경과 시간 (lock_manager, log_writer, log_applier 등) | 8 | 17 | 낮음 |
| 6 | C gettimeofday 엔진 (page_buffer flush, xasl_cache, ddl_log, CUBRID_DEBUG) | 4 | 16 | 낮음~중간 |
| 7 | master_heartbeat gettimeofday + 매크로/구조체 | 2 | 21+ | 중간 |
| 8 | 중복 시간 유틸리티 함수 중앙화 (porting.c, perf_monitor.c 등) | 3+ | 7+ 함수 | 낮음 |
| 9 | Broker/CAS gettimeofday (별도 작업 권장) | 14+ | 50+ | 중간~높음 |

### 영향도

**Tier 0~5 (이번 변경 대상, 22개 파일):**
...
- **pthread_cond_timedwait**: `wakeup_cond`, `readers_ok`, `gc_cond`가 CLOCK_MONOTONIC으로 전환됨.
  NTP 조정 시 대기 시간 왜곡이 제거됨. `pthread_condattr_setclock`은 초기화 시 1회만 실행되므로 성능 영향 없음.
  `pthread_cond_timedwait` 자체의 성능은 CLOCK_REALTIME과 CLOCK_MONOTONIC 간 동일 (같은 커널 syscall `futex` 사용)
- **lock_manager**: deadlock 검출 간격과 lock wait timeout이 monotonic 기반으로 변경.
  `lockwait_stime`(INT64 msec)의 계산 방식만 변경, 타입과 의미는 동일
- **log_writer**: log flush 간격 체크가 monotonic 기반. `last_flush_time` 멤버가 `struct timespec`로 변경
- **log_applier**: replication commit 간격 체크가 monotonic 기반. `time_commit`과 함수 시그니처가 `struct timespec`로 변경
- **log_manager**: `log_Clock_msec` 글로벌 시계가 monotonic 기반. daemon이 주기적으로 갱신하는 coarse msec 시계
- **주의**: GCC 8의 `std::condition_variable`은 내부적으로 `system_clock` 기반이므로,
  ...
  이 제약은 코드에 인라인 주석으로 문서화함

**Tier 6~8 (별도 작업):**

- **Tier 6 (page_buffer flush, xasl_cache, ddl_log, CUBRID_DEBUG)**: `struct timeval` → `struct timespec` 자료형 전환이
  기존 유틸리티 함수(`timeval_add_msec`, `timeval_diff_in_msec`, `perfmon_diff_timeval`, `LOG_GET_ELAPSED_TIME`)의
  시그니처 변경을 수반하므로 Tier 8(유틸리티 중앙화)과 함께 진행
- **Tier 7 (master_heartbeat)**: 별도 프로세스(`cub_master`). 구조체 멤버 타입 변경(~10개),
  매크로 변경(`HB_GET_ELAPSED_TIME`, `HB_IS_INITIALIZED_TIME`),
  `hb_add_timeval` 리네이밍 + overflow 버그 수정 등 변경 범위가 큼
- **Tier 8 (유틸리티 중앙화)**: `porting.c/h`에 `struct timespec` 기반 유틸리티 함수 추가 필요.
  기존 `struct timeval` 유틸리티는 broker/CAS에서 사용하므로 호환성을 위해 유지
- **Tier 6~8은 Tier 8을 선행 작업으로 하여 순서대로 진행 권장**

**Tier 9 (별도 작업, Broker/CAS):**
...
**추가로 확인된 `system_clock` 경과 시간 측정 (4곳):**

| 파일:라인 | 현재 코드 | 용도 |
|----------|----------|------|
| `src/transaction/log_recovery.c:3261,3285,3883,3892` | `std::chrono::system_clock::now()` | parallel redo 인프라 셋업/실행 시간 측정 |
| `src/query/query_opfunc.c:8590` | `using bench_clock = std::chrono::system_clock` | `BENCHMARK()` SQL 함수 실행 시간 측정 |
| `src/transaction/log_manager.c:10185` | `using clock = std::chrono::system_clock` | 아카이브 로그 삭제 daemon 경과 시간 기반 주기 계산 |
| `src/connection/server_support.c:2895` | `using clock_type = std::chrono::system_clock` | worker pool 시작 경과 시간 로깅 |
...
| `src/base/util_func.c:880` | `system_clock` | epoch 시각 반환 (wall clock 용도) |
...
### 6.2 경과 시간 측정에 사용하는 주요 파일

**엔진 (src/, broker 제외):**

| 파일 | 주요 용도 | 호출 수 |
|------|----------|--------|
| `src/transaction/lock_manager.c` | deadlock 검출 간격, lock wait timeout | 4곳 |
| `src/transaction/log_writer.c` | log flush 간격 체크 | 2곳 |
| `src/transaction/log_applier.c` | replication commit 간격 체크 | 3곳 |
| `src/transaction/log_manager.c` | `log_Clock_msec` 글로벌 시계 갱신 | 2곳 |
| `src/transaction/log_tran_table.c` | SA_MODE timeout msec 계산 | 1곳 |
| `src/storage/page_buffer.c` | flush interval 시간 제한 체크 | 7곳 |
| `src/communication/network_histogram.cpp` | 네트워크 요청 경과 시간 | 2곳 |
| `src/query/xasl_cache.c` | xasl cache 정리 간격 | 1+곳 |
| `src/base/ddl_log.c` | DDL 실행 시간 | 2곳 |
| `src/transaction/log_page_buffer.c` | CUBRID_DEBUG flush 성능 측정 | 6곳 |
| `src/executables/unloaddb.c` | unload 경과 시간 출력 | 2곳 |
| `src/executables/master_heartbeat.c` | heartbeat 경과 시간 (노드/프로세스 관리) | 20곳 |
...
| `src/base/trace_event_log.c` | 이벤트 로그 타임스탬프 |
| `src/executables/cubrid_log.c` | 로그 타임스탬프 |
| `src/api/cubrid_log.c` | 로그 타임스탬프 |
| `src/transaction/log_comm.c` | 로그 타임스탬프 |
| `src/broker/cas_sql_log2.c` | SQL 로그 타임스탬프 |
| `src/broker/broker.c` | 디버그 로그 타임스탬프 |
| `src/base/util_func.c` | `srand48_r` 시드 (랜덤) |
| `src/thread/thread_entry.cpp:190` | `srand48_r` 시드 |
| `src/parser/type_checking.c:15151,15165` | `srand48` 시드 |
| `src/parser/parse_tree.c:1215` | `srand48_r` 시드 |
| `src/storage/es_common.c:107` | 고유 ID 생성 |
| `src/query/crypt_opfunc.c:1095` | 암호화 shift 시드 |
| `src/connection/tcp.c:1406` | ICMP ping 타임스탬프 |
| `src/executables/master_heartbeat.c:6773` | 이벤트 로그 시각 포맷팅 (`hb_strtime`) |
| `src/cm_common/cm_utils.c:747,768` | 랜덤 시드 |
...
### Tier 3: pthread_cond_timedwait CLOCK_MONOTONIC (7개 파일, 10곳)

**초기화 변경 (pthread_condattr_setclock):**
- [ ] `src/thread/thread_entry.cpp:166` — `wakeup_cond`에 `CLOCK_MONOTONIC` 속성 추가
- [ ] `src/thread/critical_section.c:170` — `csect->readers_ok`에 `CLOCK_MONOTONIC` 속성 추가
- [ ] `src/transaction/log_page_buffer.c:634` — `gc_cond`에 `CLOCK_MONOTONIC` 속성 추가

**timeout 구성 변경 (time(NULL)/gettimeofday → clock_gettime(CLOCK_MONOTONIC)):**
- [ ] `src/storage/page_buffer.c:7043,7979` — latch wait, BCB alloc timeout
- [ ] `src/connection/connection_sr.c:2600` — queue wait timeout
- [ ] `src/storage/double_write_buffer.cpp:1599,1740` — DWB wait timeout (gettimeofday + msec)
- [ ] `src/thread/critical_section.c:784,1027` — csect reader wait timeout
- [ ] `src/transaction/log_page_buffer.c:4065` — group commit wait timeout (gettimeofday + msec)

**GCC 8 condition_variable 주석 추가:**
- [ ] `src/thread/thread_waiter.cpp` — `wait_for`, `wait_until`에 CLOCK_REALTIME 제약 주석
- [ ] `src/thread/thread_waiter.hpp` — `condvar_wait` 템플릿에 주석
- [ ] `src/thread/thread_looper.cpp` — `waiter_arg.wait_for`에 주석
- [ ] `src/connection/connection_pool.cpp` — `cv.wait_for` 2곳에 주석

**변경하지 않는 pthread_cond_t (timedwait 미사용):**
- `flush_start_cond`, `flush_wait_cond`, `flush_end_cond` (log_page_buffer.c) — `pthread_cond_wait`만 사용
- `cdc_Gl.producer.wait_cond` (log_manager.c) — 동일
- `complete_cond` (external_sort.c) — 동일
- `waiter_cond`, `rcv`, `wcv` (file_io.c) — 동일
- `unld_cls_info.cond` (unload_object.c) — 동일
- `clt_table_cond` (broker.c) — Tier 9 (별도 작업)

### Tier 4: C++ system_clock 추가 분 (4개 파일, 4곳)
- [ ] `src/transaction/log_recovery.c:3261,3285,3883,3892` — `system_clock` → `steady_clock`
- [ ] `src/query/query_opfunc.c:8590` — `using bench_clock = system_clock` → `steady_clock`
- [ ] `src/transaction/log_manager.c:10185` — `using clock = system_clock` → `steady_clock`
- [ ] `src/connection/server_support.c:2895` — `using clock_type = system_clock` → `steady_clock`

### Tier 5: C gettimeofday 엔진 경과 시간 (8개 파일, 17곳)
- [ ] `src/transaction/lock_manager.c:1183,2180,7465,7625` — deadlock 간격, lock wait timeout
- [ ] `src/transaction/log_writer.c:1523,1572` + `log_writer.h:99` — flush 간격 체크 (`last_flush_time` struct 변경)
- [ ] `src/transaction/log_applier.c:538,6707,6719,6728,8215` — replication commit 간격
- [ ] `src/transaction/log_manager.c:10284,10538` — `log_Clock_msec` 글로벌 시계 갱신
- [ ] `src/transaction/log_tran_table.c:2894` — SA_MODE timeout msec 계산
- [ ] `src/executables/unloaddb.c:468,478` — unload 경과 시간 출력
- [ ] `src/communication/network_histogram.cpp:140,165` — 네트워크 요청 경과 시간

### Tier 6: C gettimeofday 유틸리티 의존 (4개 파일, 16곳) — Tier 8 선행 필요
- [ ] `src/storage/page_buffer.c:4142,4234,4384,4420,16496,16504,16510` — flush interval (`timeval_add_msec`, `timeval_diff_in_msec`, `perfmon_diff_timeval` 의존)
- [ ] `src/query/xasl_cache.c:358+` — xasl cache 정리 간격
- [ ] `src/base/ddl_log.c:482,1097` — DDL 실행 시간 (`logddl_timeval_diff` 의존, 구조체 멤버 변경 필요)
- [ ] `src/transaction/log_page_buffer.c:1685,1717,2646,2749,3276,3832` — CUBRID_DEBUG flush 성능 (`LOG_GET_ELAPSED_TIME` 매크로 의존)

### Tier 7: master_heartbeat (2개 파일, 21+곳) — Tier 8 선행 필요
- [ ] `src/executables/master_heartbeat.h` — `HB_GET_ELAPSED_TIME` 매크로 (tv_usec → tv_nsec), `HB_IS_INITIALIZED_TIME` 매크로, 구조체 멤버 타입 변경 (~10개)
- [ ] `src/executables/master_heartbeat.c` — `gettimeofday` → `clock_gettime(CLOCK_MONOTONIC)` (20곳), `hb_add_timeval` 리네이밍 + overflow 버그 수정, `hb_compare_timeval` 리네이밍, wall clock 유지 (L6773 `hb_strtime` 호출)

### Tier 8: 시간 유틸리티 함수 timespec 버전 추가 — Tier 6, 7의 선행 작업
- [ ] `src/base/porting.c/h` — `timespec_add_msec()`, `timespec_diff_in_msec()` 추가
- [ ] `src/base/perf_monitor.c/h` — `perfmon_diff_timespec()` 추가 (또는 기존 함수 변경)
- 기존 `struct timeval` 유틸리티는 broker/CAS에서 사용하므로 호환성을 위해 유지

### Tier 9: Broker/CAS gettimeofday — 별도 작업
...
6. **pthread_cond_timedwait 검증**: page buffer latch wait, critical section wait, group commit wait, DWB wait가 정상 동작하는지 확인. NTP step 조정 시뮬레이션(`date -s`) 후 대기 시간 왜곡 없는지 확인
7. **lock_manager 검증**: deadlock 검출 간격이 정상 동작하는지 확인. lock wait timeout이 정확히 동작하는지 확인
8. **log_writer 검증**: semisync 모드에서 flush 간격 체크가 정상 동작하는지 확인
9. **log_applier 검증**: HA replication에서 commit 간격 체크가 정상 동작하는지 확인
10. **regression test**: 전체 회귀 테스트
11. **빌드 환경 확인**: `_GLIBCXX_USE_PTHREAD_COND_CLOCKWAIT` 정의 여부 확인. 현재 GCC 8(glibc 2.28)에서는 미지원. EL9(glibc 2.34) 이상 전환 시 `condition_variable`의 monotonic 대기가 자동 활성화됨
...
### 12.5.1 엔진 + Heartbeat 전체: 중복 시간 유틸리티 전수 목록

전수 조사 결과, 코드베이스 전체에서 시간 diff/add 기능의 변형이 **10+개** 존재한다:

| 함수/매크로 | 위치 | 자료형 | 반환 | 용도 |
|------------|------|--------|------|------|
| `timeval_diff_in_msec()` | `porting.c` | `struct timeval` | INT64 (msec) | 범용 diff |
| `timeval_add_msec()` | `porting.c` | `struct timeval` | int | 범용 add |
| `timeval_to_timespec()` | `porting.c` | timeval→timespec | int | 변환 |
| `perfmon_diff_timeval()` | `perf_monitor.c` | `struct timeval` | void (diff→timeval*) | 성능 통계 diff |
| `logddl_timeval_diff()` | `ddl_log.c` | `struct timeval` | static void | DDL 감사 로그 |
| `ut_timeval_diff()` | `cas_util.c` | `struct timeval` | void (sec/msec) | broker 전용 diff |
| `ut_diff_time()` | `broker_log_util.c` | `struct timeval` | double (초) | broker 로그 diff |
| `HB_GET_ELAPSED_TIME` | `master_heartbeat.h` | `struct timeval` | double (msec) | heartbeat 매크로 |
| `LOG_GET_ELAPSED_TIME` | `log_page_buffer.c` | `struct timeval` | double (sec) | CUBRID_DEBUG 매크로 |
| `CALCULATE_ELAPSED_TIMEVAL` | `tsc_timer.c` | `struct timeval` | void (diff→timeval*) | TSC fallback 매크로 |
| `hb_add_timeval()` | `master_heartbeat.c` | `struct timeval` | void | heartbeat add (**overflow 버그**) |
| inline diff | log_writer.c, log_applier.c 등 | `struct timeval` | 다양 | 각 파일 내 중복 |

**`hb_add_timeval` overflow 버그 상세:**
```c
/* 현재 코드 — tv_usec >= 1,000,000일 때 정규화하지 않음 */
tv_p->tv_usec += ((msec % 1000) * 1000);
/* 예: msec = 1500이면 tv_usec += 500000.
 * 기존 tv_usec가 600000이면 결과 1,100,000 → timeval 규약 위반 (tv_usec < 1,000,000이어야 함) */
```

**Tier 8 작업에서의 접근:**
1. `porting.c/h`에 `struct timespec` 기반 유틸리티 추가:
   - `timespec_diff_in_msec(start, end)` → INT64
   - `timespec_add_msec(ts, msec)` → int (정규화 포함)
2. 기존 `struct timeval` 유틸리티는 broker/CAS에서 사용하므로 호환성을 위해 유지
3. 엔진 코드에서 `timeval` 유틸리티 호출을 `timespec` 버전으로 교체
4. heartbeat의 `hb_add_timeval`은 `timespec` 전환 시 overflow 버그도 함께 수정

### 12.6 결론: 중앙 정의 교체 + 개별 호출부 변경

**C++ 코드 (중앙 정의 교체로 자동 전환):**
...
| 추가 system_clock 변경 (Tier 4) | 4 | 4 | **없음** (typedef 변경) |
| **C++ 소계** | **13개 파일** | **~25줄** | **0** |

기존 설계가 clock 타입을 중앙에서 정의하고 나머지가 참조하는 구조이므로,
C++ 코드는 **기존 중앙 정의의 clock만 교체**하면 대부분 자동으로 따라간다.

**C 코드 (개별 호출부 변경 필요):**

| 작업 | 바꿀 파일 | 바꿀 곳 수 | 비고 |
|------|----------|-----------|------|
| pthread_cond_init CLOCK_MONOTONIC (Tier 3) | 3 | 3 | 초기화 시 1회, 성능 영향 없음 |
| pthread timeout 구성 변경 (Tier 3) | 5 | 7 | `time(NULL)`/`gettimeofday` → `clock_gettime` |
| gettimeofday 엔진 경과 시간 (Tier 5) | 8 | 17 | 일부 `struct timeval` → `struct timespec` 필요 |
| gettimeofday 유틸리티 의존 (Tier 6) | 4 | 16 | 유틸리티 함수 시그니처 변경 선행 필요 |
| master_heartbeat (Tier 7) | 2 | 21+ | 구조체/매크로 변경 동반 |
| 유틸리티 timespec 버전 추가 (Tier 8) | 3+ | 7+ 함수 | Tier 6, 7의 선행 작업 |
| **C 소계** | **~25개 파일** | **~70곳** | Tier 5까지 즉시 변경, Tier 6~8은 순차 진행 |

C 코드는 중앙화된 clock 정의가 없으므로 개별 호출부를 직접 변경해야 한다.
다만 Tier 5까지는 유틸리티 함수 의존 없이 단순 치환 가능하고,
Tier 6~8은 `porting.c/h`에 `struct timespec` 기반 유틸리티를 먼저 추가한 뒤 순차 진행한다.
