...
echo "#### Start. (${T_VERSION}, standalone, ${T_COUNT})"

cubrid server stop demodb

csql -u dba demodb -S -c "${T_QUERY}"

for ((i=0; i<${T_COUNT}; i++)); do
  csql -u dba demodb -S -c "${T_QUERY}" | tail -1 | awk -F '\\(|\\)' '{print $2}' | awk '{print $1}' >> ${T_OUTPUT}
done

echo "#### End.   (${T_VERSION}, standalone, ${T_COUNT})"
